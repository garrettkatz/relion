...
size_t SasMlDeviceBundle::checkFixedSizedObjects(int shares)
{
	int devCount;
	size_t BoxLimit;
...
void SasMlDeviceBundle::setupFixedSizedObjects()
{
	int devCount;
	HANDLE_ERROR(cudaGetDeviceCount(&devCount));
	if(device_id >= devCount)
...
void SasMlDeviceBundle::setupTunableSizedObjects(size_t allocationSize)
{
...
void SasMlOptimiserCuda::resetData()
{
	int devCount;
	HANDLE_ERROR(cudaGetDeviceCount(&devCount));
	if(device_id >= devCount)
...
void SasMlOptimiserCuda::doThreadExpectationSomeParticles(int thread_id)
{
#ifdef TIMING
...
//	CTOC(cudaMLO->timer,"interParticle");
	...
	if (thread_id == 0)
		baseMLO->timer.tic(baseMLO->TIMING_ESP_DIFF2_A);
            ...
            accDoExpectationOneParticle<SasMlOptimiserCuda>(this, my_ori_particle, thread_id, ptrFactory);
