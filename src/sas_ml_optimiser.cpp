...
#ifdef CUDA
#include "src/acc/cuda/sas_cuda_ml_optimiser.h"
...
{
	SasMlOptimiser *MLO = (SasMlOptimiser*) thArg.workClass;

	try
	{
#ifdef CUDA
		if (MLO->do_gpu)
			((SasMlOptimiserCuda*) MLO->cudaOptimisers[thArg.thread_id])->doThreadExpectationSomeParticles(thArg.thread_id);
		else
#endif
...
void SasMlOptimiser::usage()
{
	parser.writeUsage(std::cout);
}

void SasMlOptimiser::read(int argc, char **argv, int rank)
{
//#define DEBUG_READ
...
void SasMlOptimiser::parseContinue(int argc, char **argv)
{
#ifdef DEBUG
...
void SasMlOptimiser::parseInitial(int argc, char **argv)
{
#ifdef DEBUG_READ
    std::cerr<<"SasMlOptimiser::parseInitial Entering "<<std::endl;
#endif
...
#ifdef DEBUG_READ
    std::cerr<<"SasMlOptimiser::parseInitial Done"<<std::endl;
#endif

}


void SasMlOptimiser::read(FileName fn_in, int rank)
{
//#define DEBUG_READ
#ifdef DEBUG_READ
    std::cerr<<"SasMlOptimiser::readStar entering ..."<<std::endl;
#endif
    ...
    if (in.fail())
        REPORT_ERROR( (std::string) "SasMlOptimiser::readStar: File " + fn_in + " cannot be read." );
    ...
    if (!MD.getValue(EMDL_OPTIMISER_OUTPUT_ROOTNAME, fn_out) ||
    	...
    	REPORT_ERROR("SasMlOptimiser::readStar: incorrect optimiser_general table");
    ...
    if (do_split_random_halves &&
    	...
    	REPORT_ERROR("SasMlOptimiser::readStar: splitting data into two random halves, but rlnModelStarFile2 not found in optimiser_general table");
...
#ifdef DEBUG_READ
    std::cerr<<"SasMlOptimiser::readStar before data."<<std::endl;
#endif
...
#ifdef DEBUG_READ
    std::cerr<<"SasMlOptimiser::readStar before model."<<std::endl;
#endif
    if (do_split_random_halves)
	{
		if (rank % 2 == 1)
...
#ifdef DEBUG_READ
    std::cerr<<"SasMlOptimiser::readStar before sampling."<<std::endl;
#endif
	sampling.read(fn_sampling);

#ifdef DEBUG_READ
    std::cerr<<"SasMlOptimiser::readStar done."<<std::endl;
#endif

}


void SasMlOptimiser::write(bool do_write_sampling, bool do_write_data, bool do_write_optimiser, bool do_write_model, int random_subset)
{
	...
	if (do_write_optimiser && random_subset < 2)
	{
		...
		if (!fh)
			REPORT_ERROR( (std::string)"SasMlOptimiser::write: Cannot write file: " + fn_tmp);
...
void SasMlOptimiser::initialise()
{
#ifdef DEBUG
    std::cerr<<"SasMlOptimiser::initialise Entering"<<std::endl;
#endif
...
#ifdef DEBUG
    std::cerr<<"SasMlOptimiser::initialise Done"<<std::endl;
#endif
}

void SasMlOptimiser::initialiseGeneral(int rank)
{
	...
	if (iter == 0)
	{
    	...
    	if (mydata.MDimg.containsLabel(EMDL_CTF_MAGNIFICATION) && mydata.MDimg.containsLabel(EMDL_CTF_DETECTOR_PIXEL_SIZE))
    	{
    		FOR_ALL_OBJECTS_IN_METADATA_TABLE(mydata.MDimg)
			{
    			...
    			else if (ABS(first_angpix - my_angpix) > 0.01)
    			{
    				...
    				REPORT_ERROR("SasMlOptimiser::initialiseGeneral: ERROR inconsistent magnification and detector pixel sizes in images in input STAR file");
    			}
			}
    	...
    	if (has_magn && ABS(first_angpix - mymodel.pixel_size) > 0.01)
    	{
    		if (verb > 0 && mymodel.pixel_size > 0.)
    			std::cout << "SasMlOptimiser::initialiseGeneral: WARNING modifying pixel size from " << mymodel.pixel_size <<" to "<<first_angpix << " based on magnification information in the input STAR file" << std::endl;
	...
	std::string errstr = "SasMlOptimiser::initialiseGeneral: ERROR a solvent mask should contain values between 0 and 1 only. Fix the input for \n ";
	if(mask1)
		errstr += " \n\t --solvent_mask ";
	if(mask2)
		errstr += " \n\t --solvent_mask2 ";
...
void SasMlOptimiser::initialiseWorkLoad()
{
...
void SasMlOptimiser::calculateSumOfPowerSpectraAndAverageImage(MultidimArray<RFLOAT> &Mavg, bool myverb)
{

#ifdef DEBUG_INI
	std::cerr<<"SasMlOptimiser::calculateSumOfPowerSpectraAndAverageImage Entering"<<std::endl;
#endif
...
#ifdef DEBUG_INI
	std::cerr<<"SasMlOptimiser::calculateSumOfPowerSpectraAndAverageImage Leaving"<<std::endl;
#endif

}

void SasMlOptimiser::setSigmaNoiseEstimatesAndSetAverageImage(MultidimArray<RFLOAT> &Mavg)
{

#ifdef DEBUG_INI
	std::cerr<<"SasMlOptimiser::setSigmaNoiseEstimatesAndSetAverageImage Entering"<<std::endl;
#endif
...
#ifdef DEBUG_INI
    std::cerr<<"SasMlOptimiser::setSigmaNoiseEstimatesAndSetAverageImage Leaving"<<std::endl;
#endif

}

void SasMlOptimiser::initialLowPassFilterReferences()
{
	if (ini_high > 0.)
	{
...
void SasMlOptimiser::iterateSetup()
{
...
void SasMlOptimiser::iterateWrapUp()
{
...
void SasMlOptimiser::iterate()
{
	...
	*/
	...
	for (iter = iter + 1; iter <= nr_iter; iter++)
	{
		...
		std::cerr << "SasMlOptimiser::iterate()" << std::endl;
...
void SasMlOptimiser::expectation()
{
...
//#define DEBUG_EXP
	...
	if (do_gpu)
    {
		...
		{
			SasMlDeviceBundle *b = new SasMlDeviceBundle(this);
		...
		for (int i = 0; i < cudaOptimiserDeviceMap.size(); i ++)
		{
			...
			didSs << "RRt" << i;
			SasMlOptimiserCuda *b = new SasMlOptimiserCuda(this, (SasMlDeviceBundle*) accDataBundles[cudaOptimiserDeviceMap[i]], didSs.str().c_str());
			b->resetData();
		...
		{
			if(((SasMlDeviceBundle*)accDataBundles[i])->device_id >= devCount || ((SasMlDeviceBundle*)accDataBundles[i])->device_id < 0 )
			{
				//std::cerr << " using device_id=" << ((SasMlDeviceBundle*)accDataBundles[i])->device_id << " (device no. " << ((SasMlDeviceBundle*)accDataBundles[i])->device_id+1 << ") which is not within the available device range" << devCount << std::endl;
				CRITICAL(ERR_GPUID);
			}
			else
				HANDLE_ERROR(cudaSetDevice(((SasMlDeviceBundle*)accDataBundles[i])->device_id));
			...
			((SasMlDeviceBundle*)accDataBundles[i])->setupTunableSizedObjects(allocationSize);
		}
	}
#endif
#ifdef ALTCPU
	...
	// Use local parameters here, as also done in the same overloaded function in SasMlOptimiserMpi
	...
	if (do_gpu)
	{
		for (int i = 0; i < accDataBundles.size(); i ++)
		{
			SasMlDeviceBundle* b = ((SasMlDeviceBundle*)accDataBundles[i]);
		...
		for (int i = 0; i < cudaOptimisers.size(); i ++)
			delete (SasMlOptimiserCuda*) cudaOptimisers[i];
		...
		for (int i = 0; i < accDataBundles.size(); i ++)
		{
			...
			((SasMlDeviceBundle*)accDataBundles[i])->allocator->syncReadyEvents();
			((SasMlDeviceBundle*)accDataBundles[i])->allocator->freeReadyAllocs();

#ifdef DEBUG_CUDA
			if (((SasMlDeviceBundle*) accDataBundles[i])->allocator->getNumberOfAllocs() != 0)
			{
				printf("DEBUG_ERROR: Non-zero allocation count encountered in custom allocator between iterations.\n");
				((SasMlDeviceBundle*) accDataBundles[i])->allocator->printState();
				fflush(stdout);
				CRITICAL(ERR_CANZ);
			}
		...
		for (int i = 0; i < accDataBundles.size(); i ++)
			delete (SasMlDeviceBundle*) accDataBundles[i];
...
void SasMlOptimiser::expectationSetup()
{
#ifdef DEBUG
...
void SasMlOptimiser::expectationSetupCheckMemory(int myverb)
{
...
void SasMlOptimiser::precalculateABMatrices()
{
...
void SasMlOptimiser::expectationSomeParticles(long int my_first_ori_particle, long int my_last_ori_particle)
{
...
void SasMlOptimiser::doThreadExpectationSomeParticles(int thread_id)
{
...
void SasMlOptimiser::expectationOneParticle(long int my_ori_particle, int thread_id)
{
#ifdef TIMING
...
//#define DEBUG_ESP_MEM
	...
	std::cerr << "Entering SasMlOptimiser::expectationOneParticle" << std::endl;
...
void SasMlOptimiser::symmetriseReconstructions()
{
	for (int ibody = 0; ibody < mymodel.nr_bodies; ibody++)
	{
		if (mymodel.nr_bodies > 1 && mymodel.keep_fixed_bodies[ibody] > 0)
...
void SasMlOptimiser::applyLocalSymmetryForEachRef()
{
...
void SasMlOptimiser::makeGoodHelixForEachRef()
{
	if ( (!do_helical_refine) || (ignore_helical_symmetry) )
		return;
...
void SasMlOptimiser::maximization()
{
...
void SasMlOptimiser::maximizationOtherParameters()
{
...
void SasMlOptimiser::solventFlatten()
{
#ifdef DEBUG
	std::cerr << "Entering SasMlOptimiser::solventFlatten" << std::endl;
#endif
	...
	else
	{
		...
		if (Isolvent().computeMin() < 0. || Isolvent().computeMax() > 1.)
			REPORT_ERROR("SasMlOptimiser::solventFlatten: ERROR solvent mask should contain values between 0 and 1 only...");
	}
	...
	if (!fn_mask2.contains("None"))
	{
		...
		if (!Isolvent2().sameShape(Isolvent()))
			REPORT_ERROR("SasMlOptimiser::solventFlatten ERROR: second solvent mask is of incorrect size.");
	}
...
#ifdef DEBUG
	std::cerr << "Leaving SasMlOptimiser::solventFlatten" << std::endl;
#endif

}

void SasMlOptimiser::updateCurrentResolution()
{
#ifdef DEBUG
	std::cerr << "Entering SasMlOptimiser::updateCurrentResolution" << std::endl;
#endif
...
#ifdef DEBUG
	std::cerr << "Leaving SasMlOptimiser::updateCurrentResolution" << std::endl;
#endif

}

void SasMlOptimiser::updateImageSizeAndResolutionPointers()
{
...
void SasMlOptimiser::calculateRunningAveragesOfMovieFrames(long int my_ori_particle,
...
void SasMlOptimiser::getFourierTransformsAndCtfs(long int my_ori_particle, int ibody, int metadata_offset,
		std::vector<MultidimArray<Complex > > &exp_Fimgs,
...
//#define DEBUG_NORM
		...
		if ( (do_helical_refine) && (!ignore_helical_symmetry) )
		{
			...
			std::cerr << "SasMlOptimiser::getFourierTransformsAndCtfs()" << std::endl;
...
void SasMlOptimiser::precalculateShiftedImagesCtfsAndInvSigma2s(bool do_also_unmasked,
...
#ifdef DEBUG_HELICAL_ORIENTATIONAL_SEARCH
			std::cerr << " SasMlOptimiser::precalculateShiftedImagesCtfsAndInvSigma2s(): do_shifts_onthefly && !do_gpu" << std::endl;
#endif
		}
		else if(!(do_gpu || do_cpu))
		{
			...
			for (long int itrans = exp_itrans_min; itrans <= exp_itrans_max; itrans++)
			{
				...
				sampling.getTranslations(itrans, exp_current_oversampling, oversampled_translations_x, oversampled_translations_y, oversampled_translations_z,
						(do_helical_refine) && (!ignore_helical_symmetry), helical_rise_initial / mymodel.pixel_size, helical_twist_initial);
#ifdef DEBUG_HELICAL_ORIENTATIONAL_SEARCH
				std::cerr << "SasMlOptimiser::precalculateShiftedImagesCtfsAndInvSigma2s(): Store all translated variants of Fimg" << std::endl;
#endif
				// Then loop over all its oversampled relatives
...
bool SasMlOptimiser::isSignificantAnyParticleAnyTranslation(long int iorient, int exp_itrans_min, int exp_itrans_max, MultidimArray<bool> &exp_Mcoarse_significant)
{
...
void SasMlOptimiser::getAllSquaredDifferences(long int my_ori_particle, int ibody,  int exp_current_image_size,
...
void SasMlOptimiser::convertAllSquaredDifferencesToWeights(long int my_ori_particle, int ibody, int exp_ipass,
...
void SasMlOptimiser::storeWeightedSums(long int my_ori_particle, int ibody, int exp_current_image_size,
		int exp_current_oversampling, int metadata_offset,
		int exp_idir_min, int exp_idir_max, int exp_ipsi_min, int exp_ipsi_max,
		int exp_itrans_min, int exp_itrans_max, int exp_iclass_min, int exp_iclass_max,
		std::vector<RFLOAT> &exp_min_diff2,
...
#ifdef DEBUG_HELICAL_ORIENTATIONAL_SEARCH
											std::cerr << "SasMlOptimiser::storeWeightedSums()" << std::endl;
											if (mymodel.data_dim == 2)
											{
												std::cerr << " exp_old_offset = (" << XX(exp_old_offset[ipart]) << ", " << YY(exp_old_offset[ipart]) << ")" << std::endl;
	...
	for (long int ipart = 0; ipart < mydata.ori_particles[my_ori_particle].particles_id.size(); ipart++)
	{
		...
		if (do_norm_correction && mymodel.nr_bodies == 1)
		{
			...
			if (!(iter == 1 && do_firstiter_cc) && DIRECT_A2D_ELEM(exp_metadata, my_image_no, METADATA_NORM) > 10.)
			{
				...
				REPORT_ERROR("SasMlOptimiser::storeWeightedSums ERROR: normalization is larger than 10");
			}
			*/
...
void SasMlOptimiser::monitorHiddenVariableChanges(long int my_first_ori_particle, long int my_last_ori_particle)
{
...
void SasMlOptimiser::updateOverallChangesInHiddenVariables()
{
...
void SasMlOptimiser::calculateExpectedAngularErrors(long int my_first_ori_particle, long int my_last_ori_particle)
{
...
void SasMlOptimiser::updateAngularSampling(bool myverb)
{
	...
	else
	{
		...
		if (!do_split_random_halves)
			REPORT_ERROR("SasMlOptimiser::updateAngularSampling: BUG! updating of angular sampling should only happen for gold-standard (auto-) refinements.");
		...
		if (nr_iter_wo_resol_gain >= MAX_NR_ITER_WO_RESOL_GAIN && nr_iter_wo_large_hidden_variable_changes >= MAX_NR_ITER_WO_LARGE_HIDDEN_VARIABLE_CHANGES)
		{
			...
			else
			{
				...
				else if (mymodel.ref_dim == 2)
				{
					...
					REPORT_ERROR("SasMlOptimiser::autoAdjustAngularSampling BUG: ref_dim should be two or three");
...
void SasMlOptimiser::updateSubsetSize(bool myverb)
{
...
void SasMlOptimiser::checkConvergence(bool myverb)
{
...
void SasMlOptimiser::setMetaDataSubset(int first_ori_particle_id, int last_ori_particle_id)
{
...
void SasMlOptimiser::getMetaAndImageDataSubset(int first_ori_particle_id, int last_ori_particle_id, bool do_also_imagedata)
{
	...
	if (do_also_imagedata)
	{
		...
		{
			if (nr_images > 1)
				REPORT_ERROR("SasMlOptimiser::getMetaAndImageDataSubset ERROR: cannot get multiple images for 3D data!");
		...
		for (long int ipart = 0; ipart < mydata.ori_particles[ori_part_id].particles_id.size(); ipart++, my_image_no++)
		{
			...
			if (mymodel.data_dim == 3 && do_ctf_correction)
			{
				...
				if (!mydata.getImageNameOnScratch(part_id, fn_ctf, true))
				{
					if (!mydata.MDimg.getValue(EMDL_CTF_IMAGE, fn_ctf, part_id))
						REPORT_ERROR("SasMlOptimiser::getMetaAndImageDataSubset ERROR: cannot find rlnCtfImage for 3D CTF correction!");
				}
			}
			if (has_converged && do_use_reconstruct_images)
			{
			...
			if (do_also_imagedata)
			{
				...
				if (XSIZE(img()) != XSIZE(exp_imagedata) || YSIZE(img()) != YSIZE(exp_imagedata) )
				{
					...
					REPORT_ERROR("SasMlOptimiser::getMetaAndImageDataSubset ERROR: incorrect image size");
				}
				if (has_converged && do_use_reconstruct_images)
				{
					...
					if (XSIZE(rec_img()) != XSIZE(exp_imagedata) || YSIZE(rec_img()) != YSIZE(exp_imagedata) )
					{
						...
						REPORT_ERROR("SasMlOptimiser::getMetaAndImageDataSubset ERROR: incorrect reconstruct_image size");
					}
				}
				if (mymodel.data_dim == 3)
				{
