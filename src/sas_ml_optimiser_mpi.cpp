...
void SasMlOptimiserMpi::read(int argc, char **argv)
{
#ifdef DEBUG
    std::cerr<<"SasMlOptimiserMpi::read Entering "<<std::endl;
#endif
...
#ifdef DEBUG
    std::cerr<<"SasMlOptimiserMpi::read done"<<std::endl;
#endif

}
void SasMlOptimiserMpi::finalise()
{
	delete node;
}

void SasMlOptimiserMpi::initialise()
{

#ifdef DEBUG
    std::cerr<<"SasMlOptimiserMpi::initialise Entering"<<std::endl;
#endif
...
#ifdef DEBUG
    std::cerr<<"SasMlOptimiserMpi::initialise Done"<<std::endl;
#endif
}

void SasMlOptimiserMpi::initialiseWorkLoad()
{

	if (do_split_random_halves)
	{
		if (node->size <= 2)
			REPORT_ERROR("SasMlOptimiserMpi::initialiseWorkLoad: at least 3 MPI processes are required when splitting data into random halves");
		if (node->size % 2 == 0)
			REPORT_ERROR("SasMlOptimiserMpi::initialiseWorkLoad: the number of MPI processes must be an odd number when gold-standard seperation is applied.");
	}
	else if (node->size <= 1)
		REPORT_ERROR("SasMlOptimiserMpi::initialiseWorkLoad: at least 2 MPI processes are required, otherwise use the sequential program");
...
void SasMlOptimiserMpi::calculateSumOfPowerSpectraAndAverageImage(MultidimArray<RFLOAT> &Mavg)
{
...
void SasMlOptimiserMpi::expectation()
{
#ifdef TIMING
		timer.tic(TIMING_EXP_1);
#endif
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::expectation: Entering " << std::endl;
#endif
...
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::expectation: done" << std::endl;
#endif

}


void SasMlOptimiserMpi::combineAllWeightedSumsViaFile()
{
...
void SasMlOptimiserMpi::combineAllWeightedSums()
{
...
void SasMlOptimiserMpi::combineWeightedSumsTwoRandomHalvesViaFile()
{
	// Just sum the weighted halves from slave 1 and slave 2 and Bcast to everyone else
	if (!do_split_random_halves)
		REPORT_ERROR("SasMlOptimiserMpi::combineWeightedSumsTwoRandomHalvesViaFile BUG: you cannot combineWeightedSumsTwoRandomHalves if you have not split random halves");
...
void SasMlOptimiserMpi::combineWeightedSumsTwoRandomHalves()
{
	// Just sum the weighted halves from slave 1 and slave 2 and Bcast to everyone else
	if (!do_split_random_halves)
		REPORT_ERROR("SasMlOptimiserMpi::combineWeightedSumsTwoRandomHalves BUG: you cannot combineWeightedSumsTwoRandomHalves if you have not split random halves");
...
void SasMlOptimiserMpi::maximization()
{
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::maximization: Entering " << std::endl;
#endif
...
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::maximization: done" << std::endl;
#endif

}

void SasMlOptimiserMpi::joinTwoHalvesAtLowResolution()
{
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::joinTwoHalvesAtLowResolution: Entering " << std::endl;
#endif

	if (!do_split_random_halves)
		REPORT_ERROR("BUG: you should not be in SasMlOptimiserMpi::joinTwoHalvesAtLowResolution!");
...
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::joinTwoHalvesAtLowResolution: done" << std::endl;
#endif

}

void SasMlOptimiserMpi::reconstructUnregularisedMapAndCalculateSolventCorrectedFSC()
{
...
void SasMlOptimiserMpi::writeTemporaryDataAndWeightArrays()
{
...
void SasMlOptimiserMpi::readTemporaryDataAndWeightArraysAndReconstruct(int iclass, int ihalf)
{
	MultidimArray<RFLOAT> dummy;
...
void SasMlOptimiserMpi::compareTwoHalves()
{
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::compareTwoHalves: Entering " << std::endl;
#endif

	if (!do_split_random_halves)
		REPORT_ERROR("ERROR: you should not be in SasMlOptimiserMpi::compareTwoHalves if !do_split_random_halves");

	if (mymodel.nr_classes > 1)
		REPORT_ERROR("ERROR: you should not be in SasMlOptimiserMpi::compareTwoHalves if mymodel.nr_classes > 1");

	if (do_sgd)
		REPORT_ERROR("ERROR: you should not be in SasMlOptimiserMpi::compareTwoHalves if doing SGD");
...
#ifdef DEBUG
	std::cerr << "SasMlOptimiserMpi::compareTwoHalves: done" << std::endl;
#endif
}

void SasMlOptimiserMpi::iterate()
{
#ifdef TIMING
...
void SasMlOptimiserMpi::processMoviesPerMicrograph(int argc, char **argv)
{

	if (!(do_movies_in_batches && fn_data_movie != "" && do_skip_maximization))
		REPORT_ERROR("SasMlOptimiserMpi::processMoviesPerMicrograp BUG: you should not be here...");
